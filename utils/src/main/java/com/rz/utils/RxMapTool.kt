...
object RxMapTool {
    ...
    fun openMap(mContext: Context, gpsFrom: Gps, gpsTo: Gps, storeName: String) {
        ...
        if (RxPackageManagerTool.haveExistPackageName(mContext, RxConstants.GAODE_PACKAGE_NAME)) {
            ...
            //检测设备是否安装百度地图APP
        } else if (RxPackageManagerTool.haveExistPackageName(
                mContext,
                RxConstants.BAIDU_PACKAGE_NAME
            )
        ) {
    ...
    fun openGaodeMapToGuide(mContext: Context, gpsFrom: Gps, gpsTo: Gps, storeName: String) {
        ...
        val gps = RxLocationTool.GPS84ToGCJ02(gpsFrom.longitude, gpsFrom.latitude)
        val gps1 = RxLocationTool.GPS84ToGCJ02(gpsTo.longitude, gpsTo.latitude)
    ...
    fun openBaiduMapToGuide(mContext: Context, gps: Gps, storeName: String) {
        val intent = Intent()
        val gps1 = RxLocationTool.GPS84ToGCJ02(gps.longitude, gps.latitude)
        val gps2 = RxLocationTool.GCJ02ToBD09(gps1!!.longitude, gps1.latitude)
