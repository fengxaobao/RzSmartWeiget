...
public class SafeKeyboard {
    ...
    private Keyboard keyboardComputer;        //计算器键盘
    ...
    private boolean isComputer = false;//是否是计算器
        ...
        public void onKey(int primaryCode, int[] keyCodes) {
            try {
                ...
                if (primaryCode == Keyboard.KEYCODE_CANCEL) {
                    ...
                    switchKeyboard();
                } else if (primaryCode == 100863) {
                    if (null != keyBoardListener) {
                        hindKeyBoard();
                        keyBoardListener.inputFinish();
                ...
                if (null != mAudioMgr) {
                    mAudioMgr.playSoundEffect(SoundEffectConstants.CLICK);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public SafeKeyboard(Context mContext, LinearLayout keyboardParentView, int keyboardLayoutResId, int mSafeKeyboardViewId,
        ...
        this(mContext, keyboardParentView, keyboardLayoutResId, mSafeKeyboardViewId, rootView, scrollLayout,
                false, isPackageByScrollView, false);
    }

    public SafeKeyboard(Context mContext, LinearLayout keyboardParentView, int keyboardLayoutResId, int mSafeKeyboardViewId,
                        @NonNull View rootView, @NonNull View scrollLayout, boolean isPackageByScrollView, boolean isComputer) {
        this(mContext, keyboardParentView, keyboardLayoutResId, mSafeKeyboardViewId, rootView, scrollLayout,
                false, false, isComputer);
    }
     ...
     */
    public SafeKeyboard(Context mContext, LinearLayout keyboardParentView, int keyboardLayoutResId, int mSafeKeyboardViewId,
                        @NonNull View rootView, @NonNull View scrollLayout, boolean letterWithNum, boolean isPackageByScrollView, boolean isComputer) {
    ...
    private void initKeyboard() {
        ...
        keyboardComputer = new Keyboard(mContext, R.xml.keyboard_computer);     //实例化 计算器
        ...
        initComputerDigitKeys();
        //播放系统按键声音
        mAudioMgr = (AudioManager) mContext.getSystemService(mContext.AUDIO_SERVICE);
    ...
    private void initComputerDigitKeys() {
        randomIdCardDigitKeys = new SparseArray<>();
        List<Keyboard.Key> keys = keyboardComputer.getKeys();
        for (Keyboard.Key key : keys) {
            int code = key.codes[0];
            randomIdCardDigitKeys.put(code, key);
        }
    ...
    private void showKeyboard() {
        ...
        if (mKeyboard != null && (mKeyboard == keyboardNumber || mKeyboard == keyboardIdCard
                || mKeyboard == keyboardNumberOnly) && keyboardView.isRandomDigit() || mKeyboard == keyboardComputer) {
