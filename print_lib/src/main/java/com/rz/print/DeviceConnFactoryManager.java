...
import static com.rz.print.DeviceConnFactoryManager.CONN_METHOD.USB;
...
//        sendStateBroadcast(CONN_STATE_CONNECTED);
    ...
    private DeviceConnFactoryManager(Context context, Build build) {
        application = context;
    ...
    public static final class Build {
        ...
        public DeviceConnFactoryManager build(Context context) {
            return new DeviceConnFactoryManager(context, this);
        }
    }
...
//                e.printStackTrace();
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case Constant.abnormal_Disconnection://异常断开连接
                    Utils.toast(application, application.getString(R.string.str_disconnect));
                    break;
                case DEFAUIT_COMMAND://默认模式
                    Utils.toast(application, application.getString(R.string.default_mode));
                    break;
                case READ_DATA:
                    ...
                    String status = application.getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            currentPrinterCommand = PrinterCommand.ESC;
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            Utils.toast(application, application.getString(R.string.str_escmode));
                        } else {//查询打印机状态
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " " + application.getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " " + application.getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " " + application.getString(R.string.str_printer_error);
                                }
                                System.out.println(application.getString(R.string.str_state) + status);
                                String mode = application.getString(R.string.str_printer_printmode_esc);
                                Utils.toast(application, mode + " " + status);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            Utils.toast(application, application.getString(R.string.str_tscmode));
                        } else {
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(application.getString(R.string.str_state) + status);
                                String mode = application.getString(R.string.str_printer_printmode_tsc);
                                Utils.toast(application, mode + " " + status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            Utils.toast(application, application.getString(R.string.str_cpclmode));
                        } else {
                            if (cnt == 1) {
                                System.out.println(application.getString(R.string.str_state) + status);
                                ...
                                String mode = application.getString(R.string.str_printer_printmode_cpcl);
                                Utils.toast(application, mode + " " + status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
    ...
    private void sendStateBroadcast(int state) {
        ...
        intent.putExtra(DEVICE_ID, id);
        application.sendBroadcast(intent);//此处若报空指针错误，需要在清单文件application标签里注册此类，参考demo
    }
